...
9. **Integridad** — botón "Verificar integridad de puntos" con auditoría en dos niveles:
   - **Nivel 1:** suma todos los documentos de `puntos` (grupo + eliminatoria + especial) agrupados por uid y los compara con el `total` guardado en `clasificacion`.
   - **Nivel 2:** recalcula desde cero los puntos de cada jugador leyendo directamente `resultados`/`predicciones` (grupos), `resultados_elim`/`predicciones_elim` (eliminatorias) y `config/general`/`pred_especiales` (MVP, goleador, campeón, subcampeón — este último deducido de `resultados_elim.final_1`), aplicando las mismas funciones de cálculo que usa la app en producción (`calcularPuntosPartido` de `resultados.js` y `calcularPuntosPartidoElim` de `resultados_elim.js`, ambas exportadas para este fin).
   - Tabla con: suma calculada, total guardado, recalculado, diferencia, estado (✓ coincide / ⚠️ discrepancia / sin datos) y una columna de detalle que lista los `partido_id` concretos donde el valor guardado no coincide con el recalculado (ej. `F3: 1→3`).
   - Si eliminatorias aún no tienen resultados confirmados, o no hay MVP/goleador oficial ni final confirmada, se muestra un aviso informativo aclarando que esa parte de la auditoría aún no tiene datos reales que comparar (para no interpretar "sin discrepancias" como "ya verificado" cuando en realidad ese tramo simplemente no se ha ejercitado todavía).
   - **Solo lectura, no modifica nada en Firestore.** Pensada para poder auditar la clasificación periódicamente sin depender de revisar Firestore a mano.
...
- ✅ Sección admin "Integridad": compara `puntos` vs `clasificacion` y además recalcula desde cero desde resultados/predicciones para verificar que las reglas de puntuación se aplicaron bien, señalando los partidos concretos con discrepancia
...
| 13 | 20 jun 2026 | `js/admin.js`, `js/resultados.js`, `js/resultados_elim.js`, `i18n/es.json`, `i18n/en.json` | Ampliada la sección "Integridad" con un segundo nivel de auditoría: recálculo desde cero de los puntos de cada jugador a partir de `resultados`/`predicciones` (grupos), `resultados_elim`/`predicciones_elim` (eliminatorias) y `config/general`/`pred_especiales` (MVP, goleador, campeón, subcampeón). Exportadas `calcularPuntosPartido` (de `resultados.js`) y `calcularPuntosPartidoElim`/`equiposCoincidenElim` (de `resultados_elim.js`) para que `admin.js` las reutilice sin duplicar la lógica de puntuación. Nueva columna "Recalculado" y columna de detalle que lista los `partido_id` concretos con discrepancia (formato `F3: 1→3`). Aviso informativo si eliminatorias o especiales (campeón/subcampeón/MVP/goleador) aún no tienen datos reales que auditar. Nuevas claves i18n: `admin.integrity.recalculated/detail/partialAudit/noElimData/noSpecialsData`. |
